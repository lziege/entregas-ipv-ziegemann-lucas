...
export (Color) var hp_cooldown_color: Color
...
func _handle_move_input() -> void:
	move_direction = int(Input.is_action_pressed("Move right")) - int(Input.is_action_pressed("Move left"))
	if move_direction != 0:
...
func sum_hp(amount: int) -> void:
	...
	if hp != max_hp:
		body.self_modulate = lerp(Color.black, hp_cooldown_color, max_hp - hp + 1)
	else:
		body.self_modulate = lerp(Color.black, hp_cooldown_color, 1)
